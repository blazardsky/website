...
const getSkewSide = (num,withMargins=true) => {
  ...
  const sideStyle = {
    'even' : `skew-x-6 -skew-y-3 rotate-3 ${withMargins && '-mr-12'}`,
    'odd'  : `-skew-x-6 skew-y-3 -rotate-3 ${withMargins && '-ml-12'}`
  }
...
const ExpCard = ({index, expData}) => {
  ...
  return (
    <div className={`relative origin-center mt-6 py-12 px-16 w-fit h-fit flex flex-col items-center gap-6 border-2 border-white text-white bg-gray-900 ${getSkewSide(index)}`}>
        ...
        <div className="block max-w-prose">
            ...
            {
              categories.map((cat) => (
                <CategoryBoucingPoint category={cat} bounce={false} explicit={true} />
              ))
            }
            </div>
